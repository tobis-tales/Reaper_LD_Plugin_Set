...
3. **Windows — built, tested offline, never run on Windows (2026-07-15).** The code is done: DLLs bundled (`extensions/Windows/`, ReaImGui 0.10.0.5 x64+x86, js_ReaScriptAPI 1.310 x64+x86), `PLATFORMS` maps `Win64`/`Win32` to the right folder, `xattr` is fenced behind `is_windows()`, and the `lipo` arch check has a pure-Lua PE-header twin (`pe_arch`) because Windows ships no lipo. `installer_test.lua` covers both Win scenarios and `pe_arch_test.lua` reads the real DLLs — but **no line of this has run inside REAPER on Windows**. Until Tobi does that, the tutorials still say "tested on macOS" and must keep saying it. Provenance is settled: every bundled macOS binary was verified byte-for-byte against the upstream release (Codeberg v0.10.0.5, ReaExtensions v1.310), and the DLLs come from those same releases.
