...
**A test's premise can expire.** Bundling the Windows DLLs made `installer_test.lua` fail on "unknown platform falls back to browse" — a scenario that used `os = "Win64"` to mean "a platform we ship nothing for". True when written, false the moment we shipped Windows builds. The code was right and the test was stale; it now says `Linux64`, which we genuinely do not bundle. **When a test fails right after you taught the code something new, suspect the test's assumption first.**

**Two reporting bugs found while adding Windows (2026-07-15), both of the same family — a check that could not fail:**
- The API-existence check shelled out to `grep -cx %q`, and the ImGui pattern starts with `-API_`. grep read it as a flag, printed "Invalid argument", and returned zero hits — so **every `ImGui_*` name was reported as a typo**, and had never actually been verified. Fixed with `grep -cx -- %q`.
- The summary printed `ALL PASS` while exiting 1 on an unknown API, because only `fails` fed the print but `fails` *and* `#unknown` fed the exit code. That reads as a broken test rather than the real finding it was, and is how a genuine signal gets ignored. Both halves are printed now.

**Mutation-check new tests before trusting them.** Each of these was proven by breaking the code on purpose and confirming the test went red — `pe_arch_test.lua`'s "PE body but no MZ stub" case exists *only* because deleting the `MZ` check left the suite green (every other malformed input is caught by the `PE\0\0` check regardless).
...
`steelblue_install.lua` runs **inside REAPER**; `build-package.sh [mac|win|all]` wraps the folder into `dist/steelblue-LD-Plugin-Set-macOS.dmg` and `-Windows.zip`. The wrapper is only packaging — the install itself has to happen in-process, because `AddRemoveReaScript` exists only there. Verified prior art: Tobi's old `ReaperRenameMarkersInstaller` was an .exe that installed the extensions and still had to tell users to "Load ReaScript..." by hand. That gap is the installer's whole reason to exist.

**This is why there is no Windows .exe, and why asking for one is reasonable but wrong.** An external installer cannot register an action or open the shortcut dialog; it could only drop the DLLs in place, leaving every REAPER-side step for the user to do by hand — exactly what the 2025 .exe had to do. It would add an unsigned-binary warning and a build chain we cannot test, and remove nothing. Both platforms ship the same Lua and the same four steps; only the wrapper differs (`.dmg` vs `.zip`).
...
The whole flow was run for real, on a REAPER wiped back to nothing: disk image → `Load ReaScript…` → both extensions installed from the bundle → four actions registered → four shortcuts assigned → quit → reopen → plugins work. Not just the harness.

**How the blank slate was made, and how to get back:** REAPER's entire state is per-user under `~/Library/Application Support/REAPER` (plus `~/Library/Preferences/com.cockos.reaper.plist`). Rename both aside with REAPER closed and the next start is indistinguishable from a fresh install. Tobi's real setup — 3482 actions, 3706 scripts, his REAPER licence and a paid X-Raym repo key — is parked in `REAPER-backup-20260715-174011`; renaming it back restores everything. A second macOS user account gives the same blank slate without touching anything, and is the safer route for anyone who has not deliberately decided to start over.

**A mounted disk image does not follow the file underneath it.** Rebuilding the .dmg while an older copy is still mounted means REAPER keeps loading the old installer, and it looks exactly like the change had no effect. Cost one confusing test round. Always `hdiutil detach` before re-testing.

## Open Priorities

1. **Startup dependency check in the four plugins** — still open. They degrade silently: three fall back to plain dialogs, and Rename quietly renumbers wrongly without js_ReaScriptAPI. The installer covers install time, but someone who removes an extension later gets no word from the plugins themselves.
2. **The installer registers itself as an action, pointing at the disk image.** `Load ReaScript…` does that automatically, so after ejecting there is a dead `/Volumes/…` entry in the Action List. It could remove itself at the end with `AddRemoveReaScript(false, …)`.
3. **Windows — works. Confirmed by Tobi on a wiped Windows REAPER, 2026-07-15.** Full run through, same as the macOS one: install → both extensions → four actions → shortcuts → quit → reopen. The tutorials now say "tested on macOS and Windows", which they had earned. Details: DLLs bundled (`extensions/Windows/`, ReaImGui 0.10.0.5 x64+x86, js_ReaScriptAPI 1.310 x64+x86), `PLATFORMS` maps `Win64`/`Win32` to the right folder, `xattr` is fenced behind `is_windows()`, and the `lipo` arch check has a pure-Lua PE-header twin (`pe_arch`) because Windows ships no lipo. Provenance is settled: every bundled macOS binary was verified byte-for-byte against the upstream release (Codeberg v0.10.0.5, ReaExtensions v1.310), and the DLLs come from those same releases — note the **GitHub `v0.10` tag is an older build than `v0.10.0.5`**, so never grab binaries by tag name alone.
   - **Still unproven: the 32-bit path.** Tobi's machine is 64-bit, so `Win32` → `reaper_imgui-x86.dll` has only ever been exercised by `installer_test.lua`, never by REAPER. It is the same code path with a different table entry, so the risk is low — but "low" is not "tested", and nobody should claim otherwise. 32-bit REAPER is essentially extinct; if it ever costs us anything, dropping the x86 builds is a reasonable answer.
