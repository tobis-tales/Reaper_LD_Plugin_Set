...
3. **Windows.** Untested, and nothing claims it. The Lua is portable, but the installer shells out to `/usr/bin/lipo` and `/usr/bin/xattr`, and `build-dmg.sh` is macOS-only. Only macOS builds are bundled; both projects publish Windows builds, and Tobi has older ones in `../ReaperRenameMarkersInstaller/`.
