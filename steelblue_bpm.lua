-- steelblue_bpm.lua
-- The Live BPM Analyzer without a window around it.
--
-- Usage:
--   local BPM = BOOT.load_module(folder, "steelblue_bpm.lua", TITLE)
--   local panel = BPM.create({ title = TITLE })
--   ...
--   panel.tick()                     -- EVERY frame, before the window opens
...
--   panel.after_frame()
--   ... and panel.release() when the window closes.
--
-- Two hosts draw this same panel, and they draw it differently:
--   * the single script "Live BPM Analyzer.lua" in its own auto-sized window
--     (layout = "window") -- the window it has always had;
--   * steelblue_workspace.lua as a compact block in the header band
--     (layout = "compact") plus popup() behind the ">>>" button.
-- The logic must not be copied into a second place, because a copy is a thing
-- that rots (AGENTS.md, 2026-07-15).
--
-- Everything below create() is the analyzer as it stood in the single script:
-- the item pick from 1.1.3, the DSP core, the incremental live reader and the
-- stepper from phase 1c, and the item-preserving "Set project tempo". The
-- function bodies were MOVED, not rewritten -- the five BPM suites
-- (bpm_accuracy, bpm_busy, bpm_confidence_validate, bpm_incremental,
-- bpm_item_pick) still dofile the single script, and they are the proof.
--
-- What belongs here: the analysis, the state it keeps, and both layouts.
-- What does NOT belong here: the window, the footer, and the popup's frame --
-- the host opens and closes the popup, this module only fills it.

local M = {}

M.VERSION = "1.0"

-- The DSP calibration. None of it is per-instance, and none of it is
-- user-editable -- the numbers the user can change (bpm_min, bpm_max,
-- window_seconds) live in create() with the rest of the state.
...
-- One analyzer. `env.title` is taken for symmetry with the other three panel
-- modules; this one has no message box and no fallback dialog to put it in.
--
-- Nothing here may touch `reaper` at creation time: the five BPM suites load
-- the single script with no fake REAPER at all and read the test hook out of
-- it, so create() has to be pure setup.
function M.create(env)
  ...
  -- ------------------------------------------------------------- the two hosts
  --
  -- Everything above this line is the analyzer, moved here word for word.
  -- Everything below is new: the queue, the live tick the host has to call, and
  -- the two layouts.

  -- Work a button asked for, run by the host AFTER its end_window. "Set project
  -- tempo" rewrites every item in the project and the precision pass reads up
  -- to two minutes of audio; neither belongs inside an open ImGui frame. Same
  -- single-slot queue the other three panel modules use -- one frame can only
  -- produce one click.
  local pending = nil
  ...
  -- The live path: one envelope update per UPDATE_INTERVAL, otherwise one slice
  -- of the tempo search. The host calls this on EVERY frame, whatever is on
  ...
  -- The three numbers the user may change. Same widths, same order, same
  -- clamps in both layouts.
  local function draw_range_fields(ctx)
    local _
  ...
  local function on_multiply(factor)
    queue(function()
      multiply_current_bpm(factor)
    end)
  end

  -- The single script's window, exactly as it has always looked. The host adds
  -- the footer around it.
  local function draw_window(ctx, SB)
  ...
  -- The confidence bar in the workspace header: 46 x 5 px, DRAWN and centred on
  -- the row rather than submitted at its own height -- a 5 px item in a row of
  -- 23 px controls sits at the top and pulls the whole block out of line.
  local COMPACT_METER_W = 46
  local COMPACT_METER_H = 5

  local function frame_height(ctx)
  ...
  local function draw_compact_meter(ctx, SB, fraction)
    ...
    if dl and type(x) == "number" and type(y) == "number" then
      local top = y + ((height - COMPACT_METER_H) / 2)
    ...
    reaper.ImGui_Dummy(ctx, COMPACT_METER_W, height)
  end

  -- The block in the workspace's header band: the number, the confidence bar,
  ...
  local function draw_compact(ctx, SB)
    ...
    -- The read-out never goes blank. With the switch off it stays on the last
    -- value and turns grey: that is what "the analysis is paused" looks like,
    -- where an empty field would look like a broken plugin.
    local value = current_bpm and string.format("%.2f", current_bpm) or "--.--"
    local value_color = (current_bpm and live_update) and SB.color.text or SB.color.text_muted
    reaper.ImGui_TextColored(ctx, value_color, value)
    reaper.ImGui_SameLine(ctx)
    ...
    return SB.button(ctx, "\u{203A}\u{203A}\u{203A}", 0, 0) and true or false
  end

  -- What sits behind ">>>": everything the header block has no room for. The
  -- host opens and closes the popup; this only fills it. Returns true when
  -- "Close" was pressed.
  --
  -- The workspace footer belongs to the open tab, so the analyzer's own status
  -- line -- including the "this looks like a timecode track" warning from
  -- 1.1.3 -- has nowhere else to go and stands at the bottom of this popup.
  local function draw_popup(ctx, SB)
    SB.section(ctx, "Tempo")
    SB.label(ctx, "Raw          " .. (raw_bpm and string.format("%.2f", raw_bpm) or "--.--"))
    SB.label(ctx, string.format("Confidence %.0f%%", confidence * 100))
    SB.label(ctx, string.format("Raw confidence  %.0f%%", raw_confidence * 100))
    ...
    if SB.primary_button(ctx, "Set project tempo", 145) then
      queue(set_project_tempo)
    end
    ...
    local close_pressed = SB.button(ctx, "Close", 70)

    reaper.ImGui_Separator(ctx)
    reaper.ImGui_TextWrapped(ctx, status_line())

    return close_pressed and true or false
  end

  -- Draws the panel into a frame the host has already opened.
  --
  --   opts.layout  "window"   the single script's own window (the default)
  --                "compact"  the workspace's header block; returns true when
  --                           ">>>" was pressed
  local function frame(ctx, SB, opts)
    opts = opts or {}

    if opts.layout == "compact" then
      return draw_compact(ctx, SB)
    end
  ...
  local function popup(ctx, SB)
    return draw_popup(ctx, SB)
  end

  -- Outside the frame: safe to touch the project. The host calls this after its
  -- end_window, never inside the frame.
  ...
  local function status_for_host()
    return status_line()
  end

  -- The accessor is a REAPER resource, not a Lua one: it has to be handed back
  -- when the window that opened it goes away.
  local function release()
    release_accessor()
  end

  -- Exactly the exports the single script used to publish. The five BPM suites
  -- set BPM_ANALYZER_TEST and dofile "Live BPM Analyzer.lua"; the host copies
  -- this table into theirs. Drop one entry and a suite goes red -- that is what
  -- makes them the proof that the analyzer only moved.
  local function test_hook()
    return {
      sample_rate = SAMPLE_RATE,
      hop_size = HOP_SIZE,
      build_onset_envelope = build_onset_envelope,
      energies_for_samples = energies_for_samples,
      envelope_from_energies = envelope_from_energies,
      trim_energies = trim_energies,
      frames_for_seconds = frames_for_seconds,
      read_live_envelope = read_live_envelope,
      release_accessor = release_accessor,
      reader = reader,
      frame_size = FRAME_SIZE,
      update_interval = UPDATE_INTERVAL,
      set_window_seconds = function(seconds) window_seconds = seconds end,
      estimate_bpm = estimate_bpm,
      start_estimate = start_estimate,
      comb_score = comb_score,
      tempo_prior = tempo_prior,
      decimate_envelope = decimate_envelope,
      refine_period = refine_period,
      -- Item picking: which item the analyzer listens to, and what it says about it.
      pick_audio_item = pick_audio_item,
      looks_like_timecode = looks_like_timecode,
