...
- **Rename selected markers** — a **Legend** button next to the preview explains every part of the syntax, one line each. In the single window it is a popup; in the docked tab it takes the tab over until you close it, because a docker is not tall enough for a popup that size.
- **MIDI notes to project markers** — a **Ruler lanes** section of its own, with **Replace existing markers in these lanes** next to it, and a plain sentence in that spot when the REAPER you are running is older than 7.72.
- **Copy Markers** — a list of every marker in the project with a tick box next to each. It mirrors the Region/Marker Manager's selection until you tick something yourself; **Use selection** goes back to following it, **Clear selection** empties it, and what is ticked is what gets copied. Target fields you leave empty follow the edit cursor; type into one and it stays where you put it.
- **Live BPM Analyzer** — **Precision analyze** reads its audio a piece at a time while REAPER keeps running, with a progress bar in place of the confidence bar, instead of freezing the window until it is done.
